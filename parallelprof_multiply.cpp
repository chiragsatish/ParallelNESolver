...
void matrix_multiplication_SSE(float* A, float* B, float* result, int n1, int m1, int m2, int lda, int ldb,float *xty, float *Y, int m3){
                ...
                //{     
                 for (k=0; k<m1; k++){
                         *(result+i*lda+j) += A[i*ldb+k] * B[k*lda+j];
                         if(j==0){
                                *(xty+i*lda+j) += A[i*ldb+k] * Y[k*lda+j];
                }
                }
                }
        }
